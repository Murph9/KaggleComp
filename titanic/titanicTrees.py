...
ids = pd.read_csv('test.csv', header=0)
ids = ids[ids.columns[0]]

train_data = pd.read_csv('train_noNa.csv', header=0).values

#use to quit without killing spyder (imagine what happened to me)
#raise Exception('exit')

test_data_pa = pd.read_csv('test_noNa.csv', header=0)
...
print "We predicted that " + str(output.sum()) + " survived."
